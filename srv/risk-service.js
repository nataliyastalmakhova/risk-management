...
module.exports = cds.service.impl(async function() {
    ...
    // const OnPremise = await cds.connect.to("LocalApp");
    ...
    this.on('getProducts', async (req)=> {
        return await Northwind.send('GET','Products')
    } )
    

    // this.on('getOnPremise', async (req)=> {
    //     return await OnPremise.send('GET','resource')
    // } )
    ...
    this.on("addItem", ({data:{descr, title, quantity}})=>{
        cds.db.run(INSERT ({descr: `${descr}`, title: `${title}`, quantity: `${quantity}`}) .into (Items))
    })

    this.on("getItemsByQuantity", async ({data:{quantity}})=>{
        return await cds.db.run(SELECT.from (Items) .where `quantity = ${quantity}`)
    })
